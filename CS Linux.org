...
** Minimize initrd image size
Change /etc/initramfs-tools/initramfs.conf configuration to
#+BEGIN_SRC txt
#
# initramfs.conf
...
** Raid managment
*** Creating raid0 from scratch
- Create a raid on a basis of already created partition(GPT)
#+BEGIN_SRC sh
  mdadm --create --verbose /dev/md0 --force --level=1 --raid-devices=1 /dev/sdb1
#+END_SRC
- Use mkfs to make proper filesystem on a created raid
#+BEGIN_SRC sh
  mkfs.ext4 /dev/md0
#+END_SRC
- Copy data from soon be included disk to a raid partition
#+BEGIN_SRC sh
  sudo rsync -ai /media/p1/* /media/p2/
#+END_SRC
- Grow raid with second partition
#+BEGIN_SRC sh
...
#+BEGIN_SRC sh
  mdadm --detail /dev/md-
#+END_SRC
- Unmound raid parition
#+BEGIN_SRC sh
  umount /dev/md0
#+END_SRC
- Identify raid device
#+BEGIN_SRC sh
  cat /proc/mdstat
#+END_SRC
- Stop raid, remove from configuration if needed
#+BEGIN_SRC sh
  mdadm --stop /dev/md0
  zile /etc/mdadm/mdadm.conf
#+END_SRC
- Remove device from raid, remove superblocks
#+BEGIN_SRC sh
